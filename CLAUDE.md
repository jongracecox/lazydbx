...
- Reserved single keys (do not bind in def Actions): global `q p ? : J C P A`, browser `d s f t r j k o O / [ ]` (j/k = navigation, [/] = tab switching, `o` = open in browser when the def implements `resource.WebLinker`, `O` = secondary web link when it implements `resource.AltWebLinker` — apps use `o` for the workspace page and `O` for the deployed app). Check `?` help for the live map.
